...
survey_data1 <- 
  ...
  select(age, vote_liberal,vote_conservative, gender, language, quebec,religion,minority, income) %>% 
  filter(age >= 18) %>%
...
Our study applies the logistic regression model to estimate the probability of voting of each voter for the Liberal Party and Conservative party. Then, we use the post-stratification which divides the survey data in terms of the variables common in the census data and the survey, and yields the estimated the overall probability of voting of the Liberal Party and Conservative Party respectively. At the end, we will the compare the estimated voting probability between the two parties to decide which one is going to win over the election (i.e. the party has the higher estimated probability). In our model, to ensure the qualification of voting, we only keep the respondents that are at least 18 years old. In order to remove the influence of missing data, we delete the observations (record of respondents) which contains missing values of variables.

### Model Specifics
<Here you can describe your regression model>
Our study creates two logistic regression models to predict the proportion of voters who will vote for the Liberal Party and the Conservative Party. Some assumptions are made before constructing our logistic regression models: The voting outcome of respondents is binary, that is, the respondents either vote for the party or not, the selected categorical variables are independent in both the logistic models built for the Liberal and the Conservative. We select the following models because the variables are matched in both the census and the survey data, and the variability of the variables is similar in survey data compared to the census data as shown in the plots of Data Summary section (...). In addition, previous study by Statistics Canada proposes that the economic status, belief, and personal inborn identities are the dominant factors that potentially influence the intention of voting, so we select the following variables as predictors to investigate the probability of each voter to vote (Uppal, S.& LaRochelle-Côté S., 2012).

$$ log(\frac{p_c}{1-p_c}) = \beta_0+\beta_1 x_{age}+\beta_2 x_{sex}+\beta_3 x_{language}+\beta_4 x_{quebec}+\beta5 x_{income}+\beta6 x_{religion}+\beta7 x_{minority}$$

$$ log(\frac{p_l}{1-p_l}) = \beta_0+\beta_1 x_{age}+\beta_2 x_{gender}+\beta_3 x_{language}+\beta_4 x_{quebec}+\beta5 x_{income}+\beta6 x_{religion}+\beta7 x_{minority}$$
...
$\beta_0$ is the intercept of the model. It represents the log odds of voting for the candidate or party when all the predictor variables are at their baseline level (e.g., the youngest age, baseline gender category, default language, etc.). It's the starting point for the model's predictions.

$\beta_1$ represents the relationship between age and the log odds of voting for the candidate or party. For every one unit increase in age (typically one year), there is an expected $\beta_1$ change in the log odds of voting for the candidate or party, assuming all other variables are held constant.

$\beta_2$ quantifies the average difference in the log odds of voting for the candidate or party between two gender groups (e.g., male and female), controlling for other factors in the model. It shows how gender influences the likelihood of voting for the candidate or party.

For $\beta_3$, This coefficient measures the influence of language on the log odds of voting for the candidate or party. It reflects how changes in language proficiency or language spoken can affect voting behavior, with other factors being equal.

$\beta_4$represents the effect of being in Quebec (or not) on the log odds of voting for the candidate or party. This could reflect regional differences in voting preferences, with all other variables held constant.

For$\beta_5$, This coefficient indicates how changes in income level impact the log odds of voting for the candidate or party. It captures the relationship between economic status and voting behavior, controlling for other variables in the model.

$\beta_6$ assesses the influence of religion on the log odds of voting for the candidate or party. This factor might capture how different religious affiliations or degrees of religiosity correlate with political preferences.

Finally, $\beta_7$ measures the impact of belonging to a minority group on the log odds of voting for the candidate or party. This could reflect how membership in certain demographic or ethnic minorities influences voting patterns.
...
# Creating the Model
model1 <- glm(vote_liberal ~ age + gender + language + quebec + income + religion + minority, data=survey_data1, family = "binomial")


model2 <- glm(vote_conservative ~ age + gender + language + quebec + income + religion + minority, data=survey_data1, family = "binomial")
#summary(model)
...
# Creating the Model
model <- lm(vote_liberal ~ age, data=survey_data1)
...
census_data_counts <- census_data1 %>% group_by(age, gender, language, quebec, income, religion, minority) %>% 
  summarise(N=n())
...
result_conservative <- census_data_counts %>% 
  mutate(conservative_predict_prop = estimate*N) %>% ungroup() %>% 
  summarise(conservative_predict = sum(conservative_predict_prop)/sum(N))

kable(result_conservative)
```
...
Uppal, S. & LaRochelle-Côté S. (February, 2012). *Factors associated with voting*. Statistics Canada. https://www150.statcan.gc.ca/n1/en/pub/75-001-x/2012001/article/11629-eng.pdf?st=nuV9FTRH
\newpage
